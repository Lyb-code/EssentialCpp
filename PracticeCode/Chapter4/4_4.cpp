...
inline UserProfile::UserProfile()
    : _login("guest"), _user_level(Beginner), _times_logged(1),
      _guesses(0), _correct_guesses(0) 
{
    static int id = 0;
    char buffer[16];
    //itoa()是C标准库所提供的函数，会将整数转换为对应的ASCII字符串形式
    _itoa(id++, buffer, 10);
...
//如果不写这行，会报: undefined reference to `UserProfile::_level_map[abi:cxx11]'
...
int main() {
    UserProfile anon;
    cout << anon;
    UserProfile anon_too; //测试是否取得唯一的标识符
    cout << anon_too;
